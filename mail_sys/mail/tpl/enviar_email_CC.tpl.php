...
while ($dato = $db->fetch(PDO::FETCH_OBJ)) {
    ...
    $dias   = $diff->days . ' days ';
    $alarma = "";
    $amount = (($dato->tipo_documento) != 'credito') ? ($dato->TotalComprobante - $dato->pagado) : (($dato->TotalComprobante - $dato->pagado) * -1);

    $TOTAL[$dato->moneda] += $amount;
    ...
    if ($nuevafecha > strtotime(date('Y-m-d'))) {
        ...
        $alarma = '<i class="fa fa-fw fa-warning" ></i>';
    ...
    if ($dato->moneda == 'USD') {
        $totalDolar = $totalDolar + $amount;
        $tr .= '$' . number_format($amount, 2, ',', '.');
    } else {
